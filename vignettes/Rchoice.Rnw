...
Note that if we set the initial values all zeros, we get:
...
Using a zero vector as the initial values of the model improves efficiency by reducing the standard errors. All other qualities shown in the case of poisson, such as correlated random parameters models, hierarchical models and so on, can be applied for binary and ordered models. \\
